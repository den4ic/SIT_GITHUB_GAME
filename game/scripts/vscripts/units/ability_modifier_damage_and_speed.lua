...
--	local hero = PlayerResource:GetSelectedHeroEntity(pID)
	...
	if gold_cost ~= true and enough_lumber ~= true then
		...
		hero:RemoveModifierByName("re_modifier_speed_ability")
		hero:AddNewModifier(hero, nil, "modifier_damage_and_speed", {})
		SpendLumber(player, lumber_cost)	
    else 
		if hero:HasModifier("modifier_damage_and_speed") then
			ReturnGold(player, gold_cost)
			end
		end
	end
	...
	-- if not hero:HasModifier("modifier_damage_and_speed") then
		-- hero:AddNewModifier(hero, nil, "modifier_damage_and_speed", {})
    -- end
	caster:AddAbility("damage_and_speed_mod_two"):SetLevel(1)
	caster:RemoveAbility("damage_and_speed_mod")
end