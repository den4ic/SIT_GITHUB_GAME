...
--	local hero = PlayerResource:GetSelectedHeroEntity(pID)
	...
	if gold_cost ~= true and enough_lumber ~= true then
		...
		if not hero:HasModifier("modifier_vision_ability") then
		hero:AddNewModifier(hero, nil, "modifier_vision_ability", {})
		SpendLumber(player, lumber_cost)	
    else 
		if hero:HasModifier("modifier_vision_ability") then
			ReturnGold(player, gold_cost)
			end
		end
	end
	...
	caster:RemoveAbility("vision_mod")
end