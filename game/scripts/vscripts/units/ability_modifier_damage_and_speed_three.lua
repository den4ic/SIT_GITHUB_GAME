...
--	local hero = PlayerResource:GetSelectedHeroEntity(pID)
	...
	if gold_cost ~= true and enough_lumber ~= true then
		...
		if not hero:HasModifier("modifier_damage_and_speed_three") then
		hero:RemoveModifierByName("modifier_damage_and_speed_two")
		hero:AddNewModifier(hero, nil, "modifier_damage_and_speed_three", {})
		SpendLumber(player, lumber_cost)	
    else 
		if hero:HasModifier("modifier_damage_and_speed_three") then
			ReturnGold(player, gold_cost)
			end
		end
	end
    ...
    -- end
	caster:AddAbility("damage_and_speed_mod_four"):SetLevel(1)
	caster:RemoveAbility("damage_and_speed_mod_three")
end