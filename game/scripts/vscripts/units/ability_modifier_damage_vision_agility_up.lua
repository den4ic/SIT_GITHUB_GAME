...
--@By Den4icccc
----------------
function mod_damage_vision_agility_up(keys)
...
--	local hero = PlayerResource:GetSelectedHeroEntity(pID)
	...
	if gold_cost ~= true and enough_lumber ~= true then
		...
		if not hero:HasModifier("modifier_damage_vision_agility_up") then
		hero:RemoveModifierByName("modifier_vision_ability")
		hero:RemoveModifierByName("modifier_damage_plus_speed_up")
		hero:AddNewModifier(hero, nil, "modifier_damage_vision_agility_up", {})
		SpendLumber(player, lumber_cost)	
    else 
		if hero:HasModifier("modifier_damage_vision_agility_up") then
			ReturnGold(player, gold_cost)
			end
		end
	end
	...
	caster:RemoveAbility("damage_vision_agility_up")
end