...
if caster:HasItemInInventory("item_meat") and caster:HasItemInInventory("item_wood") and caster:HasItemInInventory("item_wood") then
	local item = nil
	local charges = 0
	local check = 0
	for i = 0, 5 do
		item = caster:GetItemInSlot(i)
		if item ~= nil then
			if item:GetAbilityName() == "item_meat" then
				if item:GetCurrentCharges() > 1 then
					check = check+1
				end
			end
			if item:GetAbilityName() == "item_wood" then
				if item:GetCurrentCharges() > 1 then
					check = check+1
				end
			end			
	...
	if check > 1 then 
		RemoveItemFromHero(caster,"item_item_meat")	
		RemoveItemFromHero(caster,"item_item_meat")		
		RemoveItemFromHero(caster,"item_wood")	
		RemoveItemFromHero(caster,"item_wood")
		RemoveItemFromHero(caster,"item_wood")		
