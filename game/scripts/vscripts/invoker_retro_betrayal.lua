--[[ ============================================================================================================
	...
	Вызывается, когда предательство исполняется. Перемещает целевой объект в другую (обычную) команду на время.
...
function invoker_retro_betrayal_on_spell_start(keys)
	...
	local betrayal_explosion = ParticleManager:CreateParticle("particles/econ/items/omniknight/hammer_ti6_immortal/omniknight_pu_ti6_heal_hammers.vpcf", PATTACH_ABSORIGIN_FOLLOW, keys.target)
	...
	if keys.target:HasModifier("modifier_invoker_retro_betrayal") then  --If the unit is already on a unique team, simply refresh the modifier's duration.  This does not trigger the modifier's OnDestroy event.
		...
		Notifications:Top(target_player, {text="#you_have_ally", duration=3, style={color="green", ["font-size"]="45px"}})
	else
...
--	if keys.target:HasItemInInventory("item_meat") then
		Notifications:Top(target_player, {text="#you_were_offered_ally", duration=3, style={color="green", ["font-size"]="45px"}})
--	end

		--keys.target:RemoveModifierByName("modifier_invoker_retro_betrayal")
...
function modifier_invoker_retro_betrayal_on_destroy(keys)
	...
	Notifications:Top(target_pid, {text="#you_refused_union", duration=3, style={color="red", ["font-size"]="45px"}})
	
end

-- -createhero omniknight enemy
