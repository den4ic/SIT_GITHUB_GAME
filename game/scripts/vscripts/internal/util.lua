...
function CheckFood( player, food_amount, notification)
  ...
  local enough = player.food + food_amount <= Clamp(player.maxFood,200,250)
  if enough ~= true and notification == true then 
    Notifications:Bottom(player:GetPlayerID(), {text="#need_more_food", duration=1, style={color="red", ["font-size"]="35px"}})
  end
  return enough
end
...
function InitAbil( hero )       -- lvlup all ability
  for i=0, hero:GetAbilityCount()-1 do
    local abil = hero:GetAbilityByIndex(i)
    if abil ~= nil and abil:GetName() == "close_advanced_buildings_menu"  then
      if hero:IsHero() and hero:GetAbilityPoints() > 0 then
        hero:UpgradeAbility(abil)
      else
        abil:SetLevel(1)
...
function InitAntUlti( hero )       -- lvlup all ability
  for i=0, hero:GetAbilityCount()-1 do
    local abil = hero:GetAbilityByIndex(i)
